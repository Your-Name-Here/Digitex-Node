...
class DigitexAPI extends EVENTS {
    constructor(opts) {
        ...
        this.orderbook = {
            spread: {
                bid: 0, ask: 0, gap: () => { return ((this.orderbook.spread.ask - this.orderbook.spread.bid)/this.tickSize)-1; }
            },
            book: { bids: [], asks: [] },
            update: (bids, asks) => {
                this.orderbook.book.bids = bids; this.orderbook.book.asks = asks;
                this.orderbook.spread.bid = Math.max(...this.orderbook.book.bids.map(e => { return e[0] }));
                this.orderbook.spread.ask = Math.min(...this.orderbook.book.asks.map(e => { return e[0] }));
            }
        };
        this.orders = [];
        this.positions = [];
        ...
        this.curGap = 0;
        setInterval(() => { this.rateLimit.current = 0;}, 1000); // Digitex api rate limit is max of 10 per second. We reset the calls to 0 every second. 
        ...
        this.send = (msg) => {
            if (this.rateLimit.current >= this.rateLimit.max || !this.connected) {
                setTimeout(() => { this.send(msg); }, this.retryMS); // If we are not at or above the current rate limit, send it, otherwise wait for this.retryMS (default: 250ms) miliseconds to resend the same msg.
                return;
            }
            //console.log('Sending', this.rateLimit.current, msg);
            this.ws.send(msg);
        ...
        this.ws.onmessage = (msg) => {
            ...
            if (channel == 'index') { this.emit('spotUpdate', data.spotPx); this.spot = data.spotPx.toFixed(0) * 1; }
            else if (typeof channel == 'string' && channel.includes('orderbook_')) {
                
                if (!this.spot) { return; }
                this.orderbook.update(data.bids, data.asks);
                if (this.spreadGap != this.curGap) { this.curGap = this.spreadGap; this.emit('gapChange', this.curGap); }
            }
            else if (channel == 'orderFilled') {
                
                this.orders = this.orders.filter(order => {
                    if (!order.is(data.origClOrdId)) { return true; } else {return false}
                });
                this.emit('orderFilled', data);
            }
            else if (channel == 'trades') {
                this.emit('trades', data.trades);
                const maxTs = Math.max.apply(Math, data.trades.map(function (o) { return o.ts; }));
                const t = this.futuresPrice;
                this.futuresPrice = data.trades.filter(trade => { return trade.ts = maxTs })[0].px;
                if(t != this.futuresPrice){this.emit('futuresPxUpdate', this.futuresPrice);}
            }
                ...
                if (data.orderStatus == 'ACCEPTED') {
                    ...
                    this.orders.forEach(order => {
                        ...
                        if (order.is(data.origClOrdId)) { order.update(data); }
                        else { this.orders.push(new Order(data)); }
                    });
                }
            }
            ...
            //Conditional Orders
                ...
                switch (data.status) {
                    ...
                    case 'CANCELLED':
                        data.conditionalOrders.forEach(o => {
                            this.conditionalOrders = this.conditionalOrders.filter(order => { return !order.is(o.clOrdId) });
            ...
            else if (JSON.parse(msg.data.toString()).status == 'error') { console.error(`Error: ${this.getError(JSON.parse(msg.data.toString()).code)}`.red); }
            else {
        ...
        setInterval(() => { this.rate.current = 0; }, 1000);
    }
    levelHasOrder(level) {
    ...
    get spreadGap() { return this.orderbook.spread.gap(); }
    get totalContractsOfOrders() { var i = 0;this.orders.forEach(o => { i += o.qty; }); return i; }
    connect() { 
        this.ws.onopen = () => {
            this.emit('connect');
            this.send(JSON.stringify({
                "id": 2,
                "method": "subscribe",
                "params": ['trades','orderbook_'+this.depth,'index'].map((topic) => { return this.symbol + '@' + topic; })
            }));
        };
        this.ws.onclose = () => { this.emit('close') };
        this.ws.onerror = (err) => { this.emit('ws-error', err) };
        this.on('connect', () => {
            this.send(JSON.stringify({
                "id": 1,
                "method": "auth",
     ...
     * 
     * @example DigitexAPI.placeOrder({side: 'LONG', qty: 10}); // Market long 10 contracts
     * 
     * @example DigitexAPI.placeOrder({side: 'LONG', qty: 10, entry: 15005}); // Limit long 10 contracts at $15,005
     * 
     */
    placeOrder(opts) {
    ...
    cancelAllOrders(opts) {
        ...
        if (opts.side) { parameters.side = opts.side }
        else if (opts.price) { parameters.px = opts.price; }
    ...
    get connected() { return this.ws.readyState == ws.OPEN; }
    get balance() { return this.trader.balance; }
    get leverage() { return; }
    UUID() {
...
class ConditionalOrder {
    constructor(opts) {
        ...
        this.symbol = opts.symbol;
        if (['LESS_EQUAL', 'GREATER_EQUAL'].includes(opts.condition)) { this.condition = opts.condition; } else { throw new Error('You must pass opts.condition of either "GREATER_EQUALS" or "LESS_EQUALS"'); }
    }
    get triggerPrice() {
        if (this.pxValue) { return this.pxValue;}
        else if (this.offset.trigger > 0) {
            return this.px - (this.tickSize * this.offset.trigger);
        } else if (this.offset.trigger < 0) { return this.px + (this.tickSize * this.offset.trigger); }
        else { return this.px }
    }
    get px() {
        if (this.price == this.futuresPrice && this.offset.ticks == undefined) { throw new Error('This conditional will execute immediatly because the px (price) is equal to the current futures price. Maybe you forgot to also pass an offset object?');}
...
class Order extends EVENTS {
    ...
    get isConditional() { if (this.triggerPrice > 0) { return true; } return false; }
    cancelPayload() {
    ...
    update(data) { // Called when an orderStatus or conOrderStatus is received
        if (data.droppedQty > 0) { this.qty = this.qty - data.droppedQty }
        else if (data.droppedQty == 0 && data.qty == 0) { this.status = 'FILLED'; this.emit('filled', this) }
        else if (data.qty > 0) { this.filledQty += data.qty }
    }
    is(id) { return this.id == id; }// Used to compare to an ID
}
class Position {
    constructor(opts) {
